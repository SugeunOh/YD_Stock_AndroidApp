...
class MyWindow(QMainWindow):
    ...
    def receive_trdata(self, screen_no, rqname, trcode, recordname, prev_next, data_len, err_code, msg1, msg2):
        if rqname == "opt10001_req":
            ...
            json_data = json_data + '\n'  # 이래야 Client-side(Android)에서 받음.
            commandExecuter.startCommand(json_data)
            ...
            for i in range(0, 15):
                ...
                contentjson['순위'] = rank
                contentjson['종목명'] = name
                contentjson['종목코드'] = code
                contentjson['프로그램순매수금액'] = program_by_volume
                contentjson['현재가'] = curr_money
                contentjson['등락기호'] = updown_icon
                contentjson['등락율'] = updown_rate
                contentlist.append(contentjson)

            outjson['내용'] = contentlist
            json_data = json.dumps(outjson)
            json_data = json_data + '\n'  # 이래야 Client-side(Android)에서 받음.
            commandExecuter.startCommand(json_data)
            ...
            for i in range(0, 60):
                ...
                contentjson['일자'] = date_info
                contentjson['시가'] = start_money
                contentjson['종가'] = curr_money
                contentjson['고가'] = high_money
                contentjson['저가'] = low_money
                contentlist.append(contentjson)
            ...
            for i in range(0, 30):
                ...
                contentjson['일자'] = date_info
                contentjson['종가'] = curr_money
                contentjson['대비기호'] = updown_icon
                contentjson['개인투자자'] = indi_holder
                contentjson['외국인투자자'] = forg_holder
                contentjson['기관계'] = orga_holder
                contentjson['금융투자'] = finance_holder
                contentjson['보험'] = insurance_holder
                contentjson['투신'] = tusin_holder
                contentjson['기타금융'] = etcFinance_holder
                contentjson['은행'] = bank_holder
                contentjson['연기금등'] = yeongi_holder
                contentjson['사모펀드'] = samofund_holder
                contentjson['국가'] = country_holder
                contentjson['기타법인'] = etcLaw_holder
                contentjson['내외국인'] = naeforg_holder
